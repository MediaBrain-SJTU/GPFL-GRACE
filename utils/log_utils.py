...
def _Get_Log_Name(args, log_num, file_name='train'):
    ...
    log_name = f"{log_num:d}-{start_time}-{file_name}-{args.model if 'model' in args else ''}"\
        + f"-{args.lr if 'lr' in args else ''}-bs{args.batch_size if 'batch_size' in args else ''}-e{args.epochs if 'epochs' in args else ''}"\
        +f"-{args.note if 'note' in args else ''}"
    return log_name

def select_log_name(args, log_num):
    sub_name = f"{args.model if 'model' in args else ''}"\
        + f"-{args.lr if 'lr' in args else ''}-bs{args.batch_size if 'batch_size' in args else ''}-e{args.epochs if 'epochs' in args else ''}"\
        +f"-{args.note if 'note' in args else ''}"
    log_list = os.listdir(log_count_path)
    for log_name in log_list:
        if sub_name in log_name and str(log_num)+'-' in log_name:
            return log_name
    return None
...
def Gen_Log_Dir(args, file_name='train', tensorboard_subdir=True, log_path=log_count_path):
